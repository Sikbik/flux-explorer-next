...
export function BlockStats({ block }: BlockStatsProps) {
  const [blockReward, setBlockReward] = useState<number>(block.reward || 0);

  // Use shared transaction counts hook
  const { counts, loading } = useBlockTransactionCounts(block);

  // Calculate regular vs node confirmations from counts
  const regularCount = counts.coinbase + counts.transfer;
  const nodeConfirmations = counts.cumulus + counts.nimbus + counts.stratus + counts.starting + counts.unknown;

  // Fetch coinbase transaction to get actual block reward
  useEffect(() => {
    const fetchBlockReward = async () => {
      if (block.tx && block.tx.length > 0) {
        try {
          ...
          // If fetch fails, use block.reward or 0
          setBlockReward(block.reward || 0);
        }
      }
    };

    fetchBlockReward();
  }, [block.tx, block.hash, block.reward]);

  const stats = [
    {
      label: "Transactions",
      value: loading && regularCount === 0
        ? block.tx.length.toLocaleString()
        : regularCount.toLocaleString(),
      subtitle: loading && regularCount === 0
        ? "Analyzing..."
        : nodeConfirmations > 0
