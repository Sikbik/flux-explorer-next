...
import { useBlockCountsOptimized } from "@/lib/api/hooks/useBlockCountsOptimized";
...
export function BlocksList() {
  ...
  // Fetch transaction counts for current page blocks using optimized hook
  // Must be called before any early returns (React hooks rule)
  const blockHashes = currentBlocks.map(block => block.hash);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {currentBlocks.map((block) => {
          const blockCount = blockCounts.find(bc => bc.hash === block.hash);
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="text-right space-y-1">
                        <div className="flex items-center gap-2 justify-end">
                          {loadingCounts || !blockCount ? (
                            <>
                              <FileText className="h-4 w-4 text-muted-foreground" />
                              <span className="font-medium">{block.txlength} txs</span>
                            </>
                          ) : (
                            <>
                              <Badge variant="secondary">{blockCount.regularTxCount}</Badge>
                              {blockCount.nodeConfirmationCount > 0 && blockCount.tierCounts && (
                                <div className="relative group">
                                  <Badge variant="outline" className="gap-1 cursor-help">
                                    <Server className="h-3 w-3" />
                                    {blockCount.nodeConfirmationCount}
                                  </Badge>
                                  {/* Hover tooltip */}
                                  <div className="absolute right-0 bottom-full mb-2 p-3 bg-card border rounded-md shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 min-w-[220px]">
                                    <div className="space-y-2 text-sm">
                                      ...
                                      {blockCount.tierCounts.cumulus > 0 && (
                                        <div className="flex items-center justify-between">
                                          ...
                                          <span className="font-bold text-pink-500">
                                            {blockCount.tierCounts.cumulus}
                                          </span>
                                        </div>
                                      )}
                                      {blockCount.tierCounts.nimbus > 0 && (
                                        <div className="flex items-center justify-between">
                                          ...
                                          <span className="font-bold text-purple-500">
                                            {blockCount.tierCounts.nimbus}
                                          </span>
                                        </div>
                                      )}
                                      {blockCount.tierCounts.stratus > 0 && (
                                        <div className="flex items-center justify-between">
                                          ...
                                          <span className="font-bold text-blue-500">
                                            {blockCount.tierCounts.stratus}
                                          </span>
                                        </div>
                                      )}
                                      {blockCount.tierCounts.starting > 0 && (
                                        <div className="flex items-center justify-between">
                                          ...
                                          <span className="font-bold text-yellow-500">
                                            {blockCount.tierCounts.starting}
                                          </span>
                                        </div>
                                      )}
                                      {blockCount.tierCounts.unknown > 0 && (
                                        <div className="flex items-center justify-between">
