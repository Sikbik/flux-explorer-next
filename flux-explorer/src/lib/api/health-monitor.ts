 ...
 * API Health Monitor
 *
 * Monitors Blockbook API health and automatically switches between
...
function getApiEndpoints(): ApiEndpoint[] {
  ...
  // Local Blockbook (highest priority)
  ...
  if (localUrl) {
    console.log(`[Health Monitor] Adding LOCAL endpoint: ${localUrl}`);
    endpoints.push({
      url: localUrl,
      type: 'local',
      priority: 1,
    ...
    console.warn('[Health Monitor] ⚠️ No local endpoint configured - will only use public API');
  }

  // Public Blockbook (fallback)
  const publicUrl = process.env.NEXT_PUBLIC_BLOCKBOOK_API_URL ||
                    'https://blockbookflux.app.runonflux.io/api/v2';
  console.log(`[Health Monitor] Adding PUBLIC endpoint: ${publicUrl}`);
  endpoints.push({
    url: publicUrl,
    type: 'public',
    priority: 2,
